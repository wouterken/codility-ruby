  ...
  all_chars.each_cons(2).with_index(1) do |(left, right), ri|
    ...
    if left != right
      ...
      until length + li >= total_length
        ...
        left_odds     = odds
        offset_counts = counts.dup
        offset        = -1

        while li + offset != last_index && li + offset >= 0 && offset < - length
          offset_ri = ri + offset + 2
          offset_li = li + offset
          offset_right = all_chars[offset_ri]
          offset_left  = all_chars[offset_li]
          right_count  = offset_counts[offset_right] -= 1
          left_count   = offset_counts[offset_left] += 1

          left_odds    += (right_count.even? ? -1 : 1) + (left_count.even? ? -1 : 1)
